...
public class MainBoardFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mainActivity.boardRecyclerView = rootView.findViewById(R.id.boardRecyclerView);
        LinearLayoutManager layoutManager = new LinearLayoutManager(getContext(), LinearLayoutManager.VERTICAL, false);
        mainActivity.boardRecyclerView.setLayoutManager(layoutManager);

        mainActivity.boardAdapter = new BoardAdapter();
        mainActivity.boardAdapter.setOnItemClickListener(new BoardItemClickListener(mainActivity));
        ArrayList<BoardInform> boardList;
        try {
            Board board = new Board(mainActivity.getApplicationContext());
            boardList = board.listBoardPage(1);
        } catch (IOException e) {
            boardList = null;
            e.printStackTrace();
        }
        mainActivity.boardAdapter.setItems(boardList);
        mainActivity.boardRecyclerView.setAdapter(mainActivity.boardAdapter);
...
class BoardItemClickListener implements OnItemClickListener {
    MainActivity mainActivity;

    public BoardItemClickListener(MainActivity mainActivity) {
        this.mainActivity= mainActivity;
    }

    @Override
    public void onItemClick(View v, int position) {
        mainActivity.readBoardInform = mainActivity.boardAdapter.getItem(position);
